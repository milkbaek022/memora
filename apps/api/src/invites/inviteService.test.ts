...
function expectApiError(action: () => unknown, expected: { code: string; statusCode: number }): void {
  try {
    action();
...
describe("activateInvite", () => {
  ...
  it("activates the main invite code with unlimited credits", () => {
    const db = createDatabase(":memory:");
    migrateDatabase(db);
    seedMainInviteCode(db, "MEMORA-MAIN");

    const result = activateInvite(db, "MEMORA-MAIN");
  ...
  it("rejects an unknown invite code", () => {
    ...
    expectApiError(() => activateInvite(db, "MISSING"), {
      code: "INVALID_INVITE",
      statusCode: 401
    });
  });
  ...
  it("rejects a disabled invite code", () => {
    ...
    expectApiError(() => activateInvite(db, "BETA-002"), {
